...
<div class=" text-center home-banner">
    <div class="card-body">
        ...
        <?php if(Auth::check()){?>
            <a href="{{route('user-dashboard', app()->getLocale())}}" class="btn btn-outline-warning btn-lg">DASHBOARD</a>
        <?php }else{?>
            <a href="{{route('signup', app()->getLocale())}}" class="btn btn-outline-warning btn-lg t">JOIN NOW</a>
        <?php }?>
